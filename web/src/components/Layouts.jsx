...
function Shell({ homeTo, brandSub, links, actions }) {
  ...
  return (
    <header className="hero-gradient sticky top-0 z-50 overflow-x-clip text-white shadow-md [&_a]:text-inherit">
      {/* Mobile — row 1: brand + theme/lang; row 2: menu + auth (Kuber pattern) */}
      <div className="md:hidden">
        ...
        <div className="mx-auto flex max-w-7xl items-center gap-2 px-3 pb-3 pt-2">
          <button
            type="button"
            onClick={() => setOpen((v) => !v)}
            className="shrink-0 rounded-lg border border-white/30 bg-white/10 p-2"
            aria-label="Menu"
          >
        ...
        {open && (
          <div className="border-t border-white/10 px-3 pb-4">
            <nav className="flex flex-col gap-1 py-2" onClick={() => setOpen(false)}>
              {links}
            </nav>
          </div>
        )}
