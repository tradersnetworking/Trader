...
function MenuPill({ enabled, href, onClick, className, title, children }) {
  if (!enabled) {
    return (
      <span title={title} className={`${pillBase} cursor-not-allowed bg-muted text-muted-foreground opacity-60`}>
        {children}
      </span>
    );
  }
...
/** Floating WhatsApp / Telegram / Contact — main marketplace (same pattern as invest portal). */
export default function MainSupportWidget() {
  const [open, setOpen] = useState(false);
  const [telegram, setTelegram] = useState("");

  useEffect(() => {
    mainApi("/public/site-config")
      .then((d) => setTelegram(d.config?.supportLinks?.telegram || ""))
      .catch(() => {});
  }, []);

  const whatsapp = buildWhatsAppUrl(
    MAIN_SUPPORT_WHATSAPP,
    "Hello AKASHYA EXIM — I need help with export/import or an order."
  );
  const telegramUrl = buildTelegramUrl(telegram);
  const close = () => setOpen(false);

  return (
    <div className="pointer-events-none fixed bottom-4 right-2.5 z-[80] flex flex-col items-end gap-1.5 md:right-4">
      {open && (
        <div className="pointer-events-auto flex flex-col items-end gap-1.5">
          ...
          <MenuPill
            ...
            title={telegramUrl ? "Chat on Telegram" : "Telegram link not configured"}
          >
            <TelegramIcon />
            Telegram
          </MenuPill>
          <a
            href={`tel:${MAIN_SUPPORT_PHONE_TEL}`}
            onClick={close}
            title={`Call ${MAIN_SUPPORT_PHONE}`}
            className={`${pillBase} bg-navy text-white hover:scale-105 hover:bg-navy/90`}
          >
            <PhoneIcon />
            Call
          </a>
      ...
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="pointer-events-auto flex h-10 w-10 items-center justify-center rounded-full shadow-lg transition hover:scale-105 active:scale-95"
