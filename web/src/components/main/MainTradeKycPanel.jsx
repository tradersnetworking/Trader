...
export default function MainTradeKycPanel() {
  ...
  useEffect(() => {
    mainApi("/trade-kyc/mine").then((d) => {
      if (d.kyc) {
        setForm((f) => ({ ...f, ...d.kyc }));
        setStatus(d.kyc.status);
      }
    }).catch(() => {});
  }, []);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setStatus(d.kyc?.status || "PENDING");
    ...
    <div className="page-stack max-w-2xl">
      ...
      <form onSubmit={submit} className="card space-y-4 p-5">
        ...
        <div className="space-y-3 border-t border-border pt-4">
          <p className="text-xs font-bold uppercase text-muted-foreground">Upload documents</p>
        ...
        {status === "APPROVED" && (
          <button type="button" className="btn-outline text-sm" onClick={() => setViewDocs(form)}>View uploaded documents</button>
        )}

        <button className="btn-gold w-full" disabled={status === "APPROVED"}>{status === "APPROVED" ? "KYC approved" : "Submit KYC"}</button>
      </form>
    </div>
  );
}
