  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`${INVEST_STAT_GRID} mb-2`}>
        ...
        <KpiStatCard tone="amber" icon="⏳" label="Pending" value={(data?.kpis?.pendingDeposits || 0) + (data?.kpis?.pendingPayouts || 0)} />
      ...
      {view === "requests" ? (
        <div className="overflow-x-auto card">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase text-slate-400 dark:bg-white/5"><tr><th className="p-3">Date</th><th className="p-3">Type</th><th className="p-3">Amount</th><th className="p-3">Method</th><th className="p-3">Status</th><th className="p-3">Ref</th></tr></thead>
            <tbody>
              {(data?.requests || []).map((r) => (
                <tr key={`${r.kind}-${r.id}`} className="border-t dark:border-white/5">
                  <td className="p-3">{dateStr(r.createdAt, true)}</td>
                  <td className="p-3">{r.type}</td>
                  <td className="p-3 font-bold">{inr(r.amount)}</td>
                  <td className="p-3">{r.method}</td>
                  <td className="p-3"><Badge status={r.status} /></td>
                  <td className="p-3 text-xs">{r.reference || r.destination || "—"}</td>
                </tr>
              ))}
              {(data?.requests || []).length === 0 && <tr><td colSpan="6" className="p-8 text-center text-slate-400">No transactions in this period.</td></tr>}
            </tbody>
          </table>
        </div>
      ) : (
        <div className="overflow-x-auto card">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase text-slate-400 dark:bg-white/5"><tr><th className="p-3">Date</th><th className="p-3">Type</th><th className="p-3">Dir</th><th className="p-3">Amount</th><th className="p-3">Balance</th><th className="p-3">Note</th></tr></thead>
            <tbody>
              {(data?.ledger || []).map((e) => (
              ...
              {(data?.ledger || []).length === 0 && <tr><td colSpan="6" className="p-8 text-center text-slate-400">No ledger entries.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
